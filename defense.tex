    ...
    \begin{frame}
     ...
     \begin{block}{Matter:}
      \begin{center}
        ...
        \multirow{3}*{\only<1->{Fermions}} & \multicolumn{3}{ c }{\only<2->{Leptons}} & \multicolumn{3}{ c }{\only<4->{Quarks}} \tabularnewline
          & \only<3->{$e^-$ & $\mu$ & $\tau$} & \only<5->{$u$ & $c$ & $t$} \tabularnewline 
          & \only<3->{$\nu_{e}$ & $\nu_{\mu}$ & $\nu_{\tau}$} & \only<5->{$d$ & $s$ & $b$ } \tabularnewline 
          
        \end{tabular}
        %\includegraphics[width = 0.6\textwidth]{Pictures/elementaryParticles.jpg}
      \end{center}
      \end{block}
      \only<6->{\begin{block}{Forces:}
      \begin{center}
        \begin{tabular}{c c c c}
        \multirow{5}*{\only<6->{Bosons}} & \only<7->{$\gamma$} & \only<7->{$\rightarrow$} & \only<7->{E.M. interaction} \tabularnewline
        & \only<8->{$Z^{0}/W^{\pm}$} & \only<8->{$\rightarrow$} & \only<8->{Weak interaction} \tabularnewline
        & \only<9->{$g$ & $\rightarrow$} & \only<9->{Strong interaction} \tabularnewline
        & \only<10->{\textcolor{gray}{graviton} & \textcolor{gray}{$\rightarrow$}} & \only<10->{\textcolor{gray}{Gravitation}} \tabularnewline
        %\only<11->{\hline}
        & \only<11->{$H$ & $\rightarrow$} & \only<11->{Higgs field} \tabularnewline
        \end{tabular}
      \end{center}
      \end{block}}
...
%-------------------------------
  ...
  \begin{frame}
    ...
    \centering{\textbf{The analysis was performed with a software developed by IPHC-Picsel: TAF (TAPI Analysis Framework). }}}
    %\grille
\end{frame}
        ...
        \begin{block}{Alignment of the device under test (DUT)}
          \begin{itemize}
            ...
            \item To align the DUT in the reference coordinates system:
              \begin{itemize}
                \scriptsize{
                \item Modify the coordinates of our DUT in the software (tilt and/or position)
...
\begin{frame}
  ...
  \vspace{-0.2cm}
  \centering{Threshold of 6 $\sigma$, fan speed <  5m/s and 1.8M events.}
  ...
  \centering{$ \sigma_U \simeq 4.2 \ \mu m \ and \ \sigma_V \simeq 4 \ \mu m$}
\end{frame}
...
\begin{frame}
  ...
  \vspace{-0.2cm}
  \centering{Threshold of 6 $\sigma$, fan speed <  5m/s, 720k events and a tilt of 36 \degres.}
  ...
  \centering{$ \sigma_U \simeq 6.1 \ \mu m \ and \ \sigma_V \simeq 3.8 \ \mu m $}
...
\begin{frame}
  ...
  \vspace{-0.2cm}
  \centering{Metrology of the module's surface (perfomed at Bristol)}
...
%-------------------------------
\begin{frame}
  \frametitle{Parametrization of the deformation}

  \centering{Possibility to parametrise the deformation with Legendre polynomials of the $7^{th}$ order .} 

  \vspace{-0.2cm}
  \begin{center}
    \includegraphics[width = 10 cm]{Pictures/RsAlign_226057_pl6_fitted.png}
  \end{center}
  ...
  \begin{tikzpicture}[overlay,remember picture, red, shift={(current page.south west)}]
    \begin{scope}
      \draw[line width = 2pt,-latex] (5.5,1.75) ellipse (1.2cm and 0.6cm);
    \end{scope}
  \end{tikzpicture}
...
\begin{frame}
  ...
  \vspace{-0.15cm}
  \centering{$\sigma_U \simeq 5.8 \ \mu m $ instead of $ \sigma_U \simeq 6.1 \ \mu m$}
